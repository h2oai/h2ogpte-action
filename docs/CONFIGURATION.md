...
| **Custom Collection (`collection_id`)**         | -            | String                                                                                                                       | Creates a duplicate collection with the same settings and files as the specified collection. The original collection remains unchanged. Any new files from the PR, issue, or comment are added to the duplicated collection. If no collection is specified, a new empty collection is created for each run. The user must explicitly instruct the agent to read the collection. This can be done by either:<ul><li>Adding a prompt when using <strong>h2ogpte</strong></li><li>Including instructions in the prompt when creating a custom workflow (see [Custom Workflows](https://github.com/h2oai/h2ogpte-action/blob/main/docs/USAGE.md#-custom-workflows))</li> </ul> |
