...
const TOKEN_EXCHANGE_SERVER_URL = "https://8iys2kexcg.execute-api.us-east-1.amazonaws.com";

export async function checkTokenExchangeServerHealth(): Promise<void> {
  const health = await fetch(
    `${TOKEN_EXCHANGE_SERVER_URL}/dev/health`,
    {
      method: "GET",
    },
  );
...
export async function getGithubAccessToken(): Promise<string> {
  ...
  const tokenResponse = await fetch(
    `${TOKEN_EXCHANGE_SERVER_URL}/dev/token`,
    {
      method: "POST",
      headers: {
